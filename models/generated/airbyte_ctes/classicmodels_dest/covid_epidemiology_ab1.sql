{{ config(
    ...
    schema = "_airbyte_classicmodels_dest",
    tags = [ "top-level-intermediate" ]
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('classicmodels_dest', '_airbyte_raw_covid_epidemiology') }}
select
...
from {{ source('classicmodels_dest', '_airbyte_raw_covid_epidemiology') }} as table_alias
-- covid_epidemiology
where 1 = 1
